...
def printNodes(start):
    ...
    while curr.getLink() != None:    # 현재 링크의 다음 링크가 있는 동안 
        curr = curr.getLink()    # curr는 다음 노드를 가리킴
        if curr.getLink() == None:  # 더 이상 다음 노드가 없으니 화살표 필요 없음
            print(curr.getData(), end=' ')
        else:
            print(curr.getData(), end='->')
...
def insertNode(findData, insertData):
    global memory, head, prev, curr # 전역번수를 가져와서 inserNode함수 안에서 쓰겠다는 선언
